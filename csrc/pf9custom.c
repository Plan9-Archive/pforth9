...
static cell_t fsval( cell_t Val )
{
	int gridfd;
	char *msgptr;

	gridfd=open("/n/hafo/stack", O_RDWR);
	if(gridfd >= 0){
		msgptr = ConvertNumberToText( Val, gVarBase, TRUE, 1 );
		write(gridfd, msgptr, strlen(msgptr));
		write(gridfd, " ", strlen(" "));
		close(gridfd);
	}
	return Val;
...
/*
static void CGrid( cell_t Val )
{
	int gridfd;
	gridfd=open("/n/chat/chat", O_RDWR);
	if(gridfd >= 0)
		write(gridfd, "pforth9 : greetings grid\n", strlen("pforth9 : greetings grid\n"));
	close(gridfd);
}
*/
...
Err LoadCustomFunctionTable( void )
{
	...
	CustomFunctionTable[2] = fsval;
	CustomFunctionTable[3] = fsnl;
    return 0;
}
...
*/
    ...
    if( err < 0 ) return err;
    err = CreateGlueToC( "FSVAL", i++, C_RETURNS_VALUE, 1 );
    if( err < 0 ) return err;
    err = CreateGlueToC( "FSNL", i++, C_RETURNS_VALUE, 1 );
    if( err < 0 ) return err;
